...
export function profileLabel(profile) {
    ...
    if (description)
        return description;
    // No description: the card is not connected, so derive from the name.
    const family = profileFamily(profile?.name);
    const codec = codecOfProfile(profile?.name);
    const codecLabel = codec ? ` · ${codec.replace('_', '-').toUpperCase()}` : '';
    if (family === 'a2dp-sink')
        return `A2DP${codecLabel}`;
    if (family === 'headset-head-unit')
        return `Headset (HFP)${codecLabel}`;
    return profile?.name ?? '';
}
